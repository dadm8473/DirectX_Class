...
CMatrix::CMatrix()
	...
	animation(false), aniTimer(0), aniTime(0), currentScene(0), maxScene(0),
	repeatAni(false), isAniEnd(false)
{
...
{
	D3DXVECTOR2 pos = position - center;

	D3DXMatrixTransformation2D(&matrix, &center, 0, &scale, &center,
		D3DXToRadian(rotation), &pos);
