...
Game g_Game;
Scene * g_OpenScene = NULL;

bool bRigidbodyRender = false;
                          ...
                          bool* pbNoFurtherProcessing, void* pUserContext )
{
	...
	case WM_MOUSEMOVE: //★★★
		if (g_Game.mouse)
		{
			g_Game.mouse->position.x = LOWORD(lParam) - g_Game.windowWidth;
			g_Game.mouse->position.y = -HIWORD(lParam);

			if (g_OpenScene)
			{
				D3DXVec2TransformCoord(&g_Game.mouse->position, &g_Game.mouse->position, &g_OpenScene->mCamera);
			}
		}
		break;
	//case WM_LBUTTONDOWN:
...
void CALLBACK OnD3D9LostDevice( void* pUserContext )
{
	...
	//MessageBoxA(DXUTGetHWND(), "TryWorld Lost Device", "TryWorld MSG Box", MB_OK);
}
