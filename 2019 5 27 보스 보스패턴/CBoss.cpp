		...
		else // isPatternOn 트루일 경우
		{
			...
			if (attackTimer >= attackTime)
			{
				...
				case 0:		// 1번 패턴
					...
					case 0:
						for (int i = 0; i < 30; i++)
							parentScene->playerBulletList.push_back(new CBullet(position, 550, 180 + (i * 3)));
						sum = 1;
						break;
					case 1:
						for (int i = 0; i < 30; i++)
				...
				case 1:		// 2번 패턴
					for (int i = 0; i < 50; i++)
						for (int j = 0; j < 5; j++)
							parentScene->playerBulletList.push_back(new CBullet(position, 550,  (144 + (j * 144))+(sum*60)  ));
